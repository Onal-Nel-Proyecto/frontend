// ================================================================
// TablaCategorias — Tabla del listado de categorías
// ================================================================
...
import { getCategorias, changeCategoriaStatus, deleteCategoria } from '../../../../services/categoriaService';
import styles from './TablaCategorias.module.css';
...
const AccionesMenu = ({ categoria, onEditar, onCambiarEstado }) => {
      ...
      {open && coords && createPortal(
        ...
        >
          ...
          <button className={styles.actionItem} onClick={() => { setOpen(false); onCambiarEstado(); }}>
            {(categoria.cat_est || categoria.estado) === 'ACTIVO' ? (
...
const TablaCategorias = ({ onEditar }) => {
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      ...
      try {
        ...
        const resp = await getCategorias(pagAct, filtros);
        if (cancel) return;
  ...
  const filtered = categorias.filter(
    (c) =>
      (c.cat_nom || c.nombre || '')?.toLowerCase().includes(search.toLowerCase()) ||
      (c.cat_desc || c.descripcion || '')?.toLowerCase().includes(search.toLowerCase())
  );

  const handleStatusConfirm = async () => {
    if (!statusTarget) return;
    const id = statusTarget.cat_id || statusTarget.id;
    const nuevoEstado = (statusTarget.cat_est || statusTarget.estado) === 'ACTIVO' ? 'INACTIVO' : 'ACTIVO';
    setStatusTarget(null);
    setStatusLoading(true);
    try {
      const resp = await changeCategoriaStatus(id, { estado: nuevoEstado });
      setStatusLoading(false);
      if (resp?.status) {
        setStatusResult({
          type: 'success',
          title: 'Estado actualizado',
          message: resp.msg || `Categoría ${nuevoEstado === 'ACTIVO' ? 'activada' : 'inactivada'} correctamente`,
  ...
  const handleDeleteConfirm = async () => {
    ...
    const id = deleteTarget.cat_id || deleteTarget.id;
    setDeleteTarget(null);
    setDeleteLoading(true);
    try {
      const resp = await deleteCategoria(id);
      setDeleteLoading(false);
      if (resp?.status) {
        setDeleteResult({
          type: 'success',
          title: 'Categoría eliminada',
          message: resp.msg || 'Categoría eliminada correctamente',
        ...
        setDeleteResult({
          type: 'error',
          title: 'Error',
          message: resp?.msg || 'Error al eliminar la categoría',
      ...
      setDeleteResult({
        type: 'error',
        title: 'Error',
        message: err?.response?.data?.error || 'No se pudo eliminar la categoría',
        onClose: () => setDeleteResult(null),
      });
    }
  };
  ...
  return (
    <div className={styles.card}>
      ...
      <div className={styles.tableWrapper}>
        ...
        <div className={styles.mobileList}>
          {loading ? (
            ...
            <p className={styles.loadingText}>No se encontraron categorías</p>
          ) : (
            filtered.map((row) => {
              const st = statusMap[row.cat_est || row.estado] || {};
              return (
                <div key={row.cat_id || row.id} className={styles.mobileCard}>
                  <div className={styles.mobileHeader}>
                    <span className={styles.cellId}>{row.cat_id || row.id}</span>
                    <span className={`${styles.badge} ${styles[st.className] || ''}`}>
                      {st.label || row.cat_est || row.estado}
                    </span>
                  </div>
      ...
      {statusTarget && (
        <Alert
          type="confirm"
          title={(statusTarget.cat_est || statusTarget.estado) === 'ACTIVO' ? '¿Inactivar categoría?' : '¿Activar categoría?'}
          message={`¿Estás seguro de ${(statusTarget.cat_est || statusTarget.estado) === 'ACTIVO' ? 'inactivar' : 'activar'} la categoría "${statusTarget.cat_nom || statusTarget.nombre}"?`}
      ...
      {deleteTarget && (
        <Alert
          ...
          message={`¿Estás seguro de eliminar la categoría "${deleteTarget.cat_nom || deleteTarget.nombre}"? Esta acción no se puede deshacer.`}
          onCancel={() => setDeleteTarget(null)}
          onConfirm={handleDeleteConfirm}
        />
      )}

      {deleteLoading && <LoadingOverlay title="Eliminando categoría…" message="Procesando la solicitud" />}
      {deleteResult && (
